...
mahasiswa::mahasiswa(std::string id, std::string nama, int dd, int mm, int yy, std::string nrp, std::string departemen, 
					 int tahunmasuk, int semesterke, int skslulus, std::string matakuliah)
		: person(id, nama, dd, mm, yy), nrp(nrp), departemen(departemen), tahunmasuk(tahunmasuk), semesterke(semesterke), skslulus(skslulus), matakuliah(matakuliah)
{
	this->ipk = 0.0;
	this->semesterke = 1;
	this->skslulus = 0;
...
std::string mahasiswa::getMatakuliah()
{
	return this->matakuliah;
}

void mahasiswa::setMatakuliah(std::string matakuliah)
{
